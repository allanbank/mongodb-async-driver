/*
 * #%L
 * JsonTest.java - mongodb-async-driver - Allanbank Consulting, Inc.
 * %%
...
public class JsonTest {
    ...
    /**
     * Test for the {@link Json#serialize(DocumentAssignable, Json.Mode)}
     * method.
     *
     * @throws JsonParseException
     *             On a test failure.
     */
    @Test
    public void testSerializeModeStrict() throws JsonParseException {

        final DocumentBuilder b = BuilderFactory.start();
        b.pushArray("a").push().add("a", new Date(0));

        final String doc = Json.serialize(b, Json.Mode.STRICT);
        assertEquals("{" + JsonSerializationVisitor.NL + "  \"a\" : ["
                + JsonSerializationVisitor.NL + "    { \"a\" : { \"$date\" : "
                + "\"1970-01-01T00:00:00.000+0000\" } }"
                + JsonSerializationVisitor.NL + "  ]"
                + JsonSerializationVisitor.NL + "}", doc);
    }
     ...
     */
    @Test
    public void testSerializeModeStrictOneLine() throws JsonParseException {

        final DocumentBuilder b = BuilderFactory.start();
        b.pushArray("a").push().add("a", new Date(0));

        final String doc = Json.serialize(b, Json.Mode.STRICT_ONE_LINE);
        assertEquals(
                "{ \"a\" : [ { \"a\" : { \"$date\" : \"1970-01-01T00:00:00.000+0000\" } } ] }",
                doc);
    }
     ...
     */
    @Test
    public void testSerializeShell() throws JsonParseException {

        final DocumentBuilder b = BuilderFactory.start();
        b.pushArray("a").push().add("a", 1);

        final String doc = Json.serialize(b, Json.Mode.SHELL);
        assertEquals("{" + JsonSerializationVisitor.NL + "  a : ["
                + JsonSerializationVisitor.NL + "    { a : 1 }"
                + JsonSerializationVisitor.NL + "  ]"
                + JsonSerializationVisitor.NL + "}", doc);
    }
     ...
     */
    @Test
    public void testSerializeShellOneLine() throws JsonParseException {

        final DocumentBuilder b = BuilderFactory.start();
        b.pushArray("a").push().add("a", 1);

        final String doc = Json.serialize(b, Json.Mode.SHELL_ONE_LINE);
        assertEquals("{ a : [ { a : 1 } ] }", doc);
    }
     ...
     */
    @Test
    public void testSerializeWriter() throws JsonParseException {
        ...
        Json.serialize(b, writer);
        assertEquals("{ a : [ { a : 1 } ] }", writer.toString());
    }
}
